...
<<<<<<< HEAD:agnpy/photomeson/kelner_log.py
...
>>>>>>> 2626d22bb6c0cea7c9f95ddc1685745ae9a9b310:agnpy/photomeson/scripts/integration_attempts/np_trapz_integration.py
    ...
    def spectrum_calculator(
        ...
        for i, g in enumerate(output_spec):
            ...
            if particle in ('electron','positron'):
                ...
                gamma = np.logspace(np.log10(gamma_limit.value), 14, 75)
            else:
                gamma_limit = g
                gamma = np.logspace(np.log10(gamma_limit.value), 14, 75)

            if particle in ('electron', 'antinu_electron'):
                eta = np.linspace(0.945,31.3,75)
            else:
                eta = np.linspace(0.3443,31.3,75)
...
if __name__ == '__main__':
    ...
    p_dist = ECPL(A, p, gamma_cut, 1e3, 1e13)
